...
(function () {
  ...
  (function(angular) {
    angular.module('stateSecurity', [])
      .run(function($injector, $rootScope, $log, ENV,$ionicModal) {
        if( $injector.has('$state') ) {
          new StateSecurityManager($rootScope, $injector.get('$state'), $log, ENV.loginStateName, ENV.alreadyLoggedInStateName,$ionicModal);
        }
      });

    function StateSecurityManager($rootScope, $state, $log, loginState, authState,$ionicModal) {
      this._log = $log;
      this._state = $state;
      ...
      this._loginModal=$ionicModal.fromTemplateUrl('templates/login.html', {
    ...
    StateSecurityManager.prototype = {
      ...
      _authRequiredRedirect: function(state, loginState, event) {
        console.log('check path authenticated?',this._authenticated);
        ...
        if (state.data && state.data.authRequired && !this._authenticated){
          ...
          this._loginModal.then(function(modal) {
            modal.show();
          });
